...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        ema_span=int(os.getenv("EMA_SPAN", "20")),
        momentum_bars=int(os.getenv("MOMENTUM_BARS", "6")),
        breakout_lookback=int(os.getenv("BREAKOUT_LOOKBACK", "6")),
        exit_lookback=int(os.getenv("EXIT_LOOKBACK", "6")),
        max_hold_bars=int(os.getenv("MAX_HOLD_BARS", "9")),
        trailing_stop_pct=float(os.getenv("TRAILING_STOP_PCT", "0.06")),
        tranche_scheme=_parse_tranche_scheme(os.getenv("TRANCHE_SCHEME", "0.35,0.35,0.30")),
        add_delay_bars=int(os.getenv("ADD_DELAY_BARS", "2")),
